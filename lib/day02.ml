...
let parse_line line = 
  String.split_on_char ',' line |> List.map parse_id
...
let part1 lines = 
  lines |> List.hd |> parse_line |> List.map (fun (left, right) -> sum_invalid_ids_in_range is_invalid_id left right)
...
let is_rotation str =
  ...
  let rec aux i =
    if i >= len then false
      ...
      if String.equal substr str then true
      else aux (i + 1)
  in
  aux 1
...
let part2 lines = 
  lines |> List.hd |> parse_line |> List.map (fun (left, right) -> sum_invalid_ids_in_range is_invalid_id2 left right)
  |> List.fold_left (+) 0 |> string_of_int
