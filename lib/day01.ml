...
let amt rot = match rot with Left v -> v | Right v -> v
...
let apply_rotation pos rot =
  pos + match rot with Left v -> Int.neg v | Right v -> v
...
let part2 lines =
  let fold_func (pos, zeros) (rotation : rotation) =
    ...
    if amount == 0 then (pos, zeros)
      ...
      let is_left = match rotation with Left _ -> true | Right _ -> false in
